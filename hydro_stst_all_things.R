...
# 適合度檢定：KS檢定
# AIC準則：選擇最適分布
# 頻率分析：2年,10年,50年,100年
# IDF製作： Horner公式無因次參數a,b,c
# --程式只到做到這裡--
# 8場颱風雨型：程式可以找大概發生日期(L122~127)，詳見雨型分析.xlsx
# 無因次單位歷線 ： 詳見雨型分析.xlsx

# 資料輸出
# pvalue.table：優選分布結果
# horner_para：Horner無因次參數a,b,c
# hydro_value：Horner降雨強度
# happend：暴雨發生日期(雨型要用)
...
for (i in 1:length(daycut)){  #以每年的最後一天當作切點，把每年的資料分割出來 
  ...
  # if(i %in% c(32, 6 ,23, 22, 15, 18, 10, 11)){ # i為暴雨發生年分，計算暴雨發生日期
  #   u <- u+1
  #   loca <- which(f==max(f))
  #   month <- floor(as.numeric(loca)/24/30+1)
  #   day <- (loca/24-30*(month-1))
  #   happend[u,1:3] <- c(i+1986, month, day)}
  
  a <- daycut[i] + 1 # 為下一年的起始日 +1
}
#
# order(m[,24],decreasing=TRUE)[1:8] # 找到前八大暴雨發生年分
...
pvalue.table <- matrix(nrow=length(candidate)+1,ncol=24)
rownames(pvalue.table) <- c("norm","lnorm","gumbel","gamma3","lgamma3","good dist")
...
write.xlsx(pvalue.table,file) #輸出檔：K-S test 的p-value 及各延時最適合機率分布
...
# 數據整理
hydro_value <- matrix(Ty,ncol=24) #降雨強度I(mm/hr)
